//! `intent-pilot`: a Pilotage control source that flies operator intent.
//!
//! A constrained-decoding language model reads each operator message and
//! answers with a destination and an arrival behaviour. A deterministic
//! sequencer flies that intent through the typed control path, as one
//! more client beside the keyboard and the gamepad. A verifier judges the
//! flight from simulator truth against the scenario's expectation, and
//! the exit status carries its verdict.
