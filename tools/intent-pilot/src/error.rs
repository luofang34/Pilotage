...
pub(crate) enum PilotError {
    ...
    /// The scenario file is not a scenario document.
    #[error("cannot parse scenario {path}")]
    ScenarioParse {
        /// The scenario path.
        path: PathBuf,
        /// The decode failure.
    ...
    /// The scenario document breaks one of its own rules.
    #[error("invalid scenario: {detail}")]
    ScenarioInvalid {
        /// The broken rule.
        detail: String,
    },
    /// The classifier process cannot start.
    #[error("cannot start classifier `{command}`")]
    ClassifierSpawn {
    ...
    /// The classifier pipe failed.
    #[error("classifier pipe failed during {stage}")]
    ClassifierIo {
    ...
    /// The classifier answered with something that is not an intent.
    #[error("classifier reply is not usable: {detail}")]
    ClassifierReply {
        /// Why the reply is refused.
        detail: String,
    },
    /// The classifier did not answer in time.
    #[error("classifier gave no reply to {stage} in {seconds} s")]
    ClassifierTimeout {
