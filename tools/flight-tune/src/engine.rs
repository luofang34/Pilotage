    ...
    P: ProposalStrategy,
{
    ...
    pub fn run_final_qualification_once_blocking(
        ...
        let outcome = final_outcome(self.journal.state().final_evaluation.as_ref());
...
fn final_outcome(evaluation: Option<&CandidateEvaluation>) -> FinalQualificationOutcome {
    match evaluation {
        Some(CandidateEvaluation::Passed { .. }) => FinalQualificationOutcome::Qualified,
        Some(CandidateEvaluation::HardGateFailed { failure, .. }) => {
