...
content="$app_root/SituationContentView.swift"
if ! grep -q 'horizontalSizeClass == .regular' "$content"; then
...
if ! grep -q 'drawsSurface: false' "$content"; then
...
if ! grep -q 'min(modesHeight, windowHeight' "$content"; then
