...
use super::capture::{DeadzoneEvidence, SamplingSource, SourceAxisRange, TimingObservation};
...
pub struct TimingCharacterization {
    ...
    /// The estimated missing report count, when each sample is a raw report.
    pub dropped_report_count: Option<u64>,
...
pub struct AxisCharacterization {
    ...
    /// The source contract range used to validate full movement.
    pub source_range: SourceAxisRange,
    /// Whether positive physical movement produced decreasing source values.
    pub invert: bool,
    /// The first source-unit excursion that proves physical direction.
    pub first_direction_excursion: f32,
    ...
    /// The selected-clock duration of the idle evidence.
    pub idle_duration_us: u64,
