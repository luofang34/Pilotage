...
fn fuel_and_battery_quantities_do_not_mix() {
    let refused = |r: Result<Endurance, AircraftError>| {
        matches!(r, Err(AircraftError::InvalidLoading { reason, .. })
            if reason == "fuel and battery quantities do not mix")
    };
    assert!(refused(endurance(&multirotor(), fuel(60.0, None), 0.0)));
    assert!(refused(endurance(&trainer(), battery(80.0, None), 0.0)));
