...
- A lease gives real-time authority for one scope, with a fencing generation.
  `AuthorityClass` orders operators, supervisors, administrators, and
  automation (ADR-0006, ADR-0010).
- The coordination server is optional. It gives identity, rendezvous, and
  entitlements. It never carries session data (ADR-0027).
...
- A **trust anchor** record names a foreign issuer, its public key, and the
  validity period of the key. The host operator installs it out of band. The
  host policy lists the installed anchors, and the policy is signed.
...
before any lease exists. It sits next to `ControlLease` in `pilotage-authority`:
...
| `class_ceiling` | The highest `AuthorityClass` that the grantee can hold |
...
role is a named set of scopes and a class ceiling. A grant can name a role.
