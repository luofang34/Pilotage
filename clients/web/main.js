...
import { H264CanvasDecoder, FOURCC_H264 } from "./video-h264.js";
...
// One lazily-created WebCodecs decoder per source for H.264 streams; a source
// that only ever carries MJPEG never constructs one.
const h264Decoders = new Map();
...
async function paintByCodec(fourcc, payload, sourceId, target, token) {
  ...
  if (fourcc === FOURCC_H264) {
    let decoder = h264Decoders.get(sourceId);
    if (!decoder) {
      decoder = new H264CanvasDecoder(target, (message) => log(message));
      h264Decoders.set(sourceId, decoder);
    }
    decoder.decode(payload, () => transportSessions.isActive(token));
    return;
  }
