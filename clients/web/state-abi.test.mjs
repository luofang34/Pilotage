...
//
// The committed golden frames in crates/pilotage-instrument-state/fixtures/
// (the codec owner's own tree, so they travel with the crate) are generated by
// `cargo xtask gen-state-fixture` from the shared Rust posture fixtures
// and pinned by the Rust codec's own tests. This suite rebuilds the same
// three states as writer input objects and requires byte equality, so
// any drift between state-abi.js and abi/v6.rs — offsets, widths,
// endianness, enum codings, presence rules, ident atoms — turns CI red
...
function goldenHex(stem) {
  const url = new URL(
    `../../crates/pilotage-instrument-state/fixtures/${stem}.hex`,
    import.meta.url,
  );
