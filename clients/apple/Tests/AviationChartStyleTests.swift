...
final class AviationChartStyleTests: XCTestCase {
    ...
    func testSharedMapKeepsOneArchiveBindingAndSeparateVisibleGroups() throws {
        ...
        let elevation = try XCTUnwrap(output["terrain"] as? [String: Any])
        XCTAssertEqual(elevation["source"] as? String, "pilotage-terrain")
        XCTAssertEqual(elevation["exaggeration"] as? Double, 1.0)
        ...
        let elevationGroups = try XCTUnwrap(metadata["maplibre:terrain-by-display-group"] as? [String: Any])
        XCTAssertEqual(Set(elevationGroups.keys), ["terrain"])
