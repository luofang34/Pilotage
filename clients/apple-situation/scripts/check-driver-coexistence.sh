...
harness_project="$source_root/platforms/apple/AeroLink.xcodeproj/project.pbxproj"
staged_project="$staged_root/platforms/apple/AeroLink.xcodeproj/project.pbxproj"
client_project="$client_root/PilotageSituation.xcodeproj/project.pbxproj"
harness_host=$(read_setting "$harness_project" AERO_LINK_HOST_BUNDLE_IDENTIFIER)
harness_driver=$(read_setting "$harness_project" AERO_LINK_DRIVER_BUNDLE_IDENTIFIER)
...
if ! cmp -s \
    "$source_root/platforms/apple/Driver/Info.plist" \
    "$staged_root/platforms/apple/Driver/Info.plist"; then
